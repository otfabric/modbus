...
func TestTCPoverTLSClient(t *testing.T) {
	...
	// expect a tls error
	_, err = client.ReadRegisters(context.Background(), 1, 0x1000, 2, InputRegister)
	if err == nil {
	...
	regs, err = client.ReadRegisters(context.Background(), 1, 0x1000, 2, InputRegister)
	if err != nil {
	...
	regs, err = client.ReadRegisters(context.Background(), 1, 0x1002, 1, HoldingRegister)
	if err != nil {
		t.Errorf("ReadRegisters() should have succeeded, got: %v", err)
	}
	if regs[0] != 0xaabb {
...
func TestTLSClientOnServerTimeout(t *testing.T) {
	...
	err = client.WriteRegister(context.Background(), 1, 3, 0x0199)
	if err != nil {
		t.Errorf("Write() should have succeeded, got: %v", err)
	}

	// attempt to read the value back: should succeed
	reg, err = client.ReadRegister(context.Background(), 1, 3, HoldingRegister)
	if err != nil {
		t.Errorf("ReadRegisters() should have succeeded, got: %v", err)
	}
	if reg != 0x0199 {
	...
	_, err = client.ReadRegister(context.Background(), 1, 3, InputRegister)
	if err == nil {
