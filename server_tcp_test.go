...
func TestTCPServerWithConcurrentConnections(t *testing.T) {
	...
	coils, err = c1.ReadCoils(context.Background(), 9, 0x0000, 2)
	if err != nil {
	...
	coils, err = c2.ReadCoils(context.Background(), 9, 0x0003, 5)
	if err != nil {
	...
	_, err = c3.ReadCoil(context.Background(), 9, 0x0001)
	if err == nil {
	...
	err = c2.WriteCoil(context.Background(), 9, 0x0001, true)
	if err != nil {
	...
	_, err = c2.ReadCoils(context.Background(), 9, 0x0003, 5)
	if err == nil {
...
func TestTCPServerCoilsAndDiscreteInputs(t *testing.T) {
	...
	coils, err = client.ReadCoils(context.Background(), 9, 0x0000, 10)
	if err != nil {
	...
	dis, err = client.ReadDiscreteInputs(context.Background(), 9, 0x0000, 10)
	if err != nil {
	...
	_, err = client.ReadDiscreteInputs(context.Background(), 9, 0x000a, 1)
	if !errors.Is(err, ErrIllegalDataAddress) {
		t.Errorf("expected ErrIllegalDataAddress, got: %v", err)
	}
	_, err = client.ReadCoils(context.Background(), 9, 0x000a, 1)
	if !errors.Is(err, ErrIllegalDataAddress) {
		t.Errorf("expected ErrIllegalDataAddress, got: %v", err)
	}
	_, err = client.ReadDiscreteInputs(context.Background(), 9, 0x8, 3)
	if !errors.Is(err, ErrIllegalDataAddress) {
		t.Errorf("expected ErrIllegalDataAddress, got: %v", err)
	}
	_, err = client.ReadCoils(context.Background(), 9, 0x8, 3)
	if !errors.Is(err, ErrIllegalDataAddress) {
		t.Errorf("expected ErrIllegalDataAddress, got: %v", err)
	}
	...
	err = client.WriteCoil(context.Background(), 9, 0x0004, true)
	if err != nil {
		t.Errorf("client.WriteCoil() should have succeeded, got: %v", err)
	}

	// make sure it has been written to
	coils, err = client.ReadCoils(context.Background(), 9, 0x0003, 3)
	if err != nil {
		t.Errorf("client.ReadCoils() should have succeeded, got: %v", err)
	}
	...
	err = client.WriteCoils(context.Background(), 9, 0x0005, []bool{
	...
	coils, err = client.ReadCoils(context.Background(), 9, 0x0005, 4)
	if err != nil {
		t.Errorf("client.ReadCoils() should have succeeded, got: %v", err)
	}
...
func TestTCPServerHoldingAndInputRegisters(t *testing.T) {
	...
	regs, err = client.ReadRegisters(context.Background(), 9, 0x0000, 10, InputRegister)
	if err != nil {
	...
	regs, err = client.ReadRegisters(context.Background(), 9, 0x0009, 1, InputRegister)
	if err != nil {
		t.Errorf("client.ReadRegisters() should have succeeded, got: %v", err)
	}
	if regs[0] != 0xa719 {
	...
	_, err = client.ReadRegisters(context.Background(), 9, 0x0001, 10, InputRegister)
	if !errors.Is(err, ErrIllegalDataAddress) {
		t.Errorf("client.ReadRegisters() should have returned ErrIllegalDataAddress, got: %v", err)
	}
	_, err = client.ReadRegisters(context.Background(), 9, 0x0000, 11, InputRegister)
	if !errors.Is(err, ErrIllegalDataAddress) {
		t.Errorf("client.ReadRegisters() should have returned ErrIllegalDataAddress, got: %v", err)
	}

	// all 10 holding registers should still be 0x0000
	regs, err = client.ReadRegisters(context.Background(), 9, 0x0000, 10, HoldingRegister)
	if err != nil {
		t.Errorf("client.ReadRegisters() should have succeeded, got: %v", err)
	}
	for i := 0; i < 10; i++ {
	...
	err = client.WriteRegister(context.Background(), 9, 0x0007, 0xfea1)
	if err != nil {
		t.Errorf("client.WriteRegister() should have succeeded, got: %v", err)
	}

	// make sure it has been written to
	regs, err = client.ReadRegisters(context.Background(), 9, 0x0005, 5, HoldingRegister)
	if err != nil {
		t.Errorf("client.ReadRegisters() should have succeeded, got: %v", err)
	}
	for i := 0; i < 5; i++ {
	...
	err = client.WriteRegisters(context.Background(), 9, 0x0001, []uint16{
	...
	// write to a single valid register (with opcode 0x06)
	err = client.WriteRegister(context.Background(), 9, 0x0000, 0x0c00)
	if err != nil {
		t.Errorf("client.WriteRegister() should have succeeded, got: %v", err)
	}
	...
	// reading addr 0x0009 (the very last register) should succeed
	regs, err = client.ReadRegisters(context.Background(), 9, 0x0009, 1, HoldingRegister)
	if err != nil {
		t.Errorf("client.ReadRegisters() should have succeeded, got: %v", err)
	}
	if regs[0] != 0x0c99 {
	...
	// reading past address 0x000a should fail
	_, err = client.ReadRegisters(context.Background(), 9, 0x0001, 10, HoldingRegister)
	if !errors.Is(err, ErrIllegalDataAddress) {
		t.Errorf("client.ReadRegisters() should have returned ErrIllegalDataAddress, got: %v", err)
	}
	_, err = client.ReadRegisters(context.Background(), 9, 0x0000, 11, HoldingRegister)
	if !errors.Is(err, ErrIllegalDataAddress) {
		t.Errorf("client.ReadRegisters() should have returned ErrIllegalDataAddress, got: %v", err)
	}
	...
	// return ErrIllegalFunction
	err = client.WriteRegisters(context.Background(), 2, 0x0005, []uint16{
		0x0000, 0x0001,
	})
	if !errors.Is(err, ErrIllegalFunction) {
		t.Errorf("client.WriteRegisters() should have returned ErrIllegalFunction, got: %v", err)
	}
	err = client.WriteRegister(context.Background(), 2, 0x0001, 0xffff)
	if !errors.Is(err, ErrIllegalFunction) {
		t.Errorf("client.WriteRegister() should have returned ErrIllegalFunction, got: %v", err)
	}
	_, err = client.ReadRegisters(context.Background(), 2, 0x0005, 1, HoldingRegister)
	if !errors.Is(err, ErrIllegalFunction) {
		t.Errorf("client.ReadRegisters() should have returned ErrIllegalFunction, got: %v", err)
	}
	_, err = client.ReadRegisters(context.Background(), 2, 0x0005, 1, InputRegister)
	if !errors.Is(err, ErrIllegalFunction) {
		t.Errorf("client.ReadRegisters() should have returned ErrIllegalFunction, got: %v", err)
	}
