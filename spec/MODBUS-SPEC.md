...
- 6 Function codes descriptions
   ...
   - 6.19 43 ( 0x2B) Encapsulated Interface Transport
      - PDU 6.20 43 / 13 (0x2B / 0x0D) CANopen General Reference Request and Response
   - 6.21 43 / 14 (0x2B / 0x0E) Read Device Identification
- 7 MODBUS Exception Responses
   - MEI TYPES Annex A (Informative): MODBUS RESERVED FUNCTION CODES, SUBCODES AND
...
responses). W hen a message is sent f rom a Client to a Server device the function code field
...
exception-function_code = [1 byte] MODBUS function code + 0x
...
```
```
Holding Registers 16 - bit word Read-W rite
This type of data can be alterable by an application
program.
```
```
...
Afterwards the MODBUS data model has to be bound to the device application ( IEC -
...
```
Read Coils 01 01 6.
Write Single Coil 05 05 6.
Write Multiple Coils 15 0F 6.
```
```
16 bits
access
...
```
```
Read Holding Registers 03 03 6.
Write Single Register 06 06 6.
Write Multiple Registers 16 10 6.
Read/Write Multiple Registers 23 17 6.
Mask Write Register 22 16 6.
Read FIFO queue 24 18 6.
```
```
File record access
```
```
Read File record 20 14 6.
Write File record 21 15 6.
```
```
Diagnostics
```
```
Read Exception status 07 07 6.
Diagnostic 08 00 - 18,20 08 6.
Get Com event counter 11 OB 6.
Get Com Event Log 12 0C 6.
Report Server ID 17 11 6.
Read device Identification 43 14 2B 6.
...
```
CANopen General Reference 43 13 2B 6.
```
## 6 Function codes descriptions
...
Function code 1 Byte 0x
Byte count 1 Byte N*
Coil Status n Byte n = N or N+
```

*N = Quantity of Outputs / 8, if the remainder is different of 0  N = N+
Error
Function code 1 Byte Function code + 0x
...
*N = Quantity of Inputs / 8 if the remainder is different of 0  N = N+
Error
...
Output Value 2 Bytes 0x0000 or 0xFF
...
turned off. The Ready watchdog timer is allowed to expire, locking the controls off. W hile the
...
```
### 6.16 22 (0x16) Mask W rite Register
...
Quantity to W rite 2 Bytes 0x0001 to 0X007 9
Write Byte Count 1 Byte 2 x N*
Write Registers Value N*x 2 Bytes
*N = Quantity to W rite
...
Quantity to W rite Hi 00 Read Registers value Lo 01
Quantity to W rite Lo 03 Read Registers value Hi 00
...
access
0x 01 : basic identification (stream access only)
0x 02 : regular identification (stream access only)
0x 03 : extended identification (stream access only)
0x 81 : basic identification (stream access and individual access)
0x 82 : regular identification (stream access and individual access)
0x 83 : extended identification(stream access and individual
access)
...
05 ACKNOW LEDGE Specialized use in conjunction with programming
commands.
...
```
0A GATEW AY PATH UNAVAILABLE Specialized use in conjunction with gateways,
